...
class FiscalSimulator:
    ...
    def import_pix_nfe(self, pix: dict, nfe: Optional[dict] = None, actor: str = "user"):
        ...
        has_sig = bool(pix.get("has_digital_signature", False)) and (nfe is None or nfe.get("has_digital_signature", False))
        ...
        event = self.journal.post_entry(
            f"Recebimento Pix + NF-e {pix.get('txid', '')}",
            lines,
